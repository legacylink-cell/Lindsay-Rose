...
- **Typography**: Headings/display = **Cormorant Garamond** (official brand serif, weights 600/700 to avoid a thin look). Body/UI = **Montserrat** (free Proxima Nova stand-in, per user's earlier choice). Loaded in index.html; mapped via tailwind `serif`/`display` (Cormorant) + `sans` (Montserrat) and index.css (h1–h4 Cormorant 600, body Montserrat). Custom `font-600/700/800` weight utilities registered in tailwind.
