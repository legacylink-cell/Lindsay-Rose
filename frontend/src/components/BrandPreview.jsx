...
const LOGOS = [
  {
    id: "A",
    file: "/logos/logo-a-house-horizontal.png",
    bg: "#202020",
    h: "h-12 md:h-14",
  ...
  {
    id: "B",
    file: "/logos/logo-b-rooftop-emblem.png",
    bg: "#271C20",
    h: "h-16 md:h-20",
    name: "Rooftop Emblem",
  ...
  {
    id: "D",
    file: "/logos/logo-d-bold-wordmark.png",
    bg: "#231F21",
    h: "h-11 md:h-12",
    name: "Bold Wordmark",
...
const HeaderMock = ({ logo }) => (
  <div className="rounded-2xl overflow-hidden ring-1 ring-black/10 shadow-lift">
    {/* Mock site header */}
    <div className="flex items-center justify-between px-5 md:px-8 py-4" style={{ backgroundColor: logo.bg }}>
      <img src={logo.file} alt={logo.name} className={`${logo.h} w-auto object-contain`} />
      <nav className="hidden lg:flex items-center gap-6">
        {NAV.map((n) => (
          <span key={n} className="text-[13px] text-brand-cream/75 font-medium">{n}</span>
        ))}
      </nav>
      <div className="flex items-center gap-3">
        <span className="hidden md:inline-flex items-center gap-1.5 text-brand-cream/85 text-sm font-semibold">
          <Phone className="w-4 h-4 text-brand-amber" /> 469-443-6903
        </span>
        <span className="inline-flex items-center bg-brand-amber text-brand-ink text-sm font-bold px-4 py-2 rounded-full">
          Get a free quote
        </span>
      </div>
    </div>
    {/* Mock hero strip for context */}
    <div className="bg-brand-cream px-5 md:px-8 py-8">
    ...
    <div className="min-h-screen bg-brand-cream text-brand-ink" data-testid="brand-preview-page">
      <header className="border-b border-black/10 bg-white/70 backdrop-blur">
        <div className="max-w-5xl mx-auto px-5 md:px-8 py-6">
          ...
          <h1 className="mt-2 font-serif text-3xl md:text-4xl font-700 text-brand-green">Logo Options — On the Live Site</h1>
          <p className="mt-3 text-brand-ink/70 max-w-2xl">
            Here’s each logo mounted in a real site header, exactly how it would appear at the top of your homepage. Tell me which letter you want (e.g. “Let’s go with Logo A”) and I’ll set it live everywhere and build a matching favicon + share image.
          </p>
        </div>
      </header>
